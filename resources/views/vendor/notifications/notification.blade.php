    ...
    <div class="grid flex-1">
        @if ($title = $getTitle())
            <x-notifications::title>
                {{ \Illuminate\Support\Str::of(__($title, \translate_notification('title', $notification->getViewData())))->markdown()->sanitizeHtml()->toHtmlString() }}
            </x-notifications::title>
        @endif
        ...
        @if ($body = $getBody())
            <x-notifications::body>
                {{ \Illuminate\Support\Str::of(__($body, \translate_notification('body', $notification->getViewData())))->markdown()->sanitizeHtml()->toHtmlString() }}
            </x-notifications::body>
        @endif
